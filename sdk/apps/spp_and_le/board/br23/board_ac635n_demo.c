...
NORFLASH_DEV_PLATFORM_DATA_BEGIN(norflash_data)
    ...
    .spi_read_width         = TCFG_HW_SPI1_MODE,
...
REGISTER_DEVICE(device_table) = {
    .name = "res_nor",
    .ops = &norfs_dev_ops,
    .priv_data = (void *) &norflash_data
};
...
void board_init()
{
    ...
    // extern int exflash_test2(void);
    // exflash_test2();
    extern void exflash_imount(void);
    exflash_imount();    
}
...
//adkey / io可以作为唤醒口保留
    ...
    port_protect(port_group, TCFG_VBAT_5V_WAKE);
#endif
...
struct sys_time current_alarm = {0};
void X_timer_scan(void *priv)
{
    ...
    unix_time = custom_mktime(&current_time);

    history_data.unix_time1_h = unix_time >> 24;   
    history_data.unix_time1_l = unix_time >> 16;   
...
void rtc_no_osc_get_time(void)
{
    T_Time app_time;
    u32 unix_time;
    unsigned long time = jiffies_msec();
    // log_info("%s[%d %d]", __func__, timer_get_ms(), time);
    u32 hour = time/(60*60*1000);
    u32 min  = (time%(60*60*1000))/(60*1000);
    u32 sec  = (time%(60*1000))/(1000);
    u32 ms   = time%(1000);
    // printf("%dhour %dmin %dsec %dms", hour, min, sec, ms);
    struct sys_time current_time = {0};
    rtc_ioctl(IOCTL_GET_SYS_TIME, (u32)&current_time);
    // g_printf("%d-%d-%d %d:%d:%d\n", current_time.year, current_time.month, current_time.day,
    //          current_time.hour, current_time.min, current_time.sec);

    unix_time = mkusatime(&current_time,8);

    // g_printf("decimal %d, HEx 0x%x",unix_time,unix_time);
    // log_info("%s[%d][%d]",__func__,gpio_read(CHARGE_STATUS_IO),gpio_read(DCDC_TEST_PORT));
...
void rtc_set_wakeup_later(void)
{
    T_ALARM tmp_alarm = {0};
    rtc_ioctl(IOCTL_GET_SYS_TIME, (u32)&tmp_alarm.time);
    tmp_alarm.en = 1;                           //��ʼ��Ĭ�ϴ�
    tmp_alarm.mode = E_ALARM_MODE_ONCE;         //������ֻ������һ��
    /* tmp_alarm.time.hour = 0; */
    tmp_alarm.time.min += 3; // = 0;
    /* tmp_alarm.time.sec  += 60; */
//    if(tmp_alarm.time.sec >= 60){
//        tmp_alarm.time.min++;
//        tmp_alarm.time.sec -= 60;
//    }
    g_printf("%s[%d-%d-%d %d:%d:%d]\n", __func__,
             tmp_alarm.time.year, tmp_alarm.time.month, tmp_alarm.time.day,
             tmp_alarm.time.hour, tmp_alarm.time.min, tmp_alarm.time.sec);
...
void board_power_init(void)
{
	...
	sys_timer_add(NULL, X_timer_scan, 1000);
