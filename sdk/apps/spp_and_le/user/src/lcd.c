...
spi_dev lcd_spi1_hdl = 1;
static spi_dev lcd_spi2_hdl = 2;


extern int spi_open(spi_dev spi);
extern int spi_send_byte(spi_dev spi, u8 byte);
extern void user_delay_us(u32 usec);
...
static const InitCode LcdInit_code[] = {
    ...
    {0xD0, 2, {0xA4, 0xA1}},
    {0xD6, 1, {0xA1}},
    {0xE0, 14, {0xF0, 0x03, 0x09, 0x03, 0x03, 0x10, 0x2D, 0x43, 0x3F, 0x33, 0x0D, 0x0E, 0x29, 0x32}},
    {0xE1, 14, {0xF0, 0x0C, 0x10, 0x0E, 0x0E, 0x0A, 0x2D, 0x33, 0x45, 0x3A, 0x14, 0x19, 0x31, 0x37}},
...
{
	spi_open(lcd_spi1_hdl);
	LCD_CS_SLECT_DISABLE();
}
...
#else
	// spi_send_byte(lcd_spi1_hdl, i);                   //NULL DMA
	lcd_spi_dma_send_byte(i);						 //HAVE DMA
#endif
...
#else
	spi_send_byte(lcd_spi1_hdl, i);

#endif
    // SPI_CS = 1;
}

void SPI_WriteData_Dma_Da(u8 *buffer, u32 size)
{ 
    unsigned char n,m;
...
#else
	// spi_dma_set_addr_for_isr(lcd_spi1_hdl,buffer, size, 1);
	spi_dma_send(lcd_spi1_hdl,buffer, size);
#endif
    // SPI_CS = 1;
}
...
void lcd_spi_dma_wait_finish()
{
    if (spi_pnd) {
        while (!spi_get_pending(lcd_spi1_hdl)) {
            wdt_clear();
        }
        spi_clear_pending(lcd_spi1_hdl);
        spi_pnd = false;
    }
}


int __lcd_spi_dma_send(spi_dev spi, void *buf, u32 len, u8 wait)
{
    int err = 0;

    if (!wait || spi_pnd) {
        lcd_spi_dma_wait_finish();
    }
    spi_dma_set_addr_for_isr(lcd_spi1_hdl, buf, len, 0);
    spi_pnd = true;
    asm("csync");
...
void lcd_spi_dma_send_byte(char dat)
{
    ...
    if (lcd_spi1_hdl) {
        err = __lcd_spi_dma_send(lcd_spi1_hdl, &_dat, 1, 1);
    }

    if (err < 0) {
        printf("spi dma send byte timeout\n");
    }
}
...
void WriteDispData(unsigned char DataH,unsigned char DataL)
{
	...
	/* DBH=DataH;
	 ...
	 WR0=0;
	 WR0=1; */
	 SPI_WriteData_Da(DataH);
	 SPI_WriteData_Da(DataL);
...
SPI_WriteData(0x00);
SPI_WriteData(0x00);
SPI_WriteData(0x13);//VPMIDP G[0] 15/16
SPI_WriteData(0x00);
SPI_WriteData(0x11);//VRP1 G[1] 10
SPI_WriteData(0x00);
SPI_WriteData(0x16);//VRP0 G[2] 21
SPI_WriteData(0x00);
SPI_WriteData(0x05);//KP7 G[3] 0
SPI_WriteData(0x00);
SPI_WriteData(0x0d);//KP6 G[4] 1
SPI_WriteData(0x00);
SPI_WriteData(0x0c);//KP5 G[5] 2
SPI_WriteData(0x00);
SPI_WriteData(0x0d);//KP4 G[6] 6
SPI_WriteData(0x00);
SPI_WriteData(0x1e);//KP3 G[7] 25
SPI_WriteData(0x00);
SPI_WriteData(0x1b);//KP2 G[8] 29
SPI_WriteData(0x00);
SPI_WriteData(0x1b);//KP1 G[9] 30
SPI_WriteData(0x00);
SPI_WriteData(0x1f);//KP0 G[10] 31
SPI_WriteData(0x00);
SPI_WriteData(0x00);
SPI_WriteData(0x00);
SPI_WriteData(0x00);
...
SPI_WriteData(0x00);
SPI_WriteData(0x00);
SPI_WriteData(0x13);//VPMIDN G[11] 15/16
SPI_WriteData(0x00);
SPI_WriteData(0x16); //VRN0 G[13] 21
SPI_WriteData(0x00);
SPI_WriteData(0x11);//VRN1 G[12] 10
SPI_WriteData(0x00);
SPI_WriteData(0x1f);//KN0 G[21] 31
SPI_WriteData(0x00);
SPI_WriteData(0x1b);//KN1 G[20] 30
SPI_WriteData(0x00);
SPI_WriteData(0x1b);//KN2 G[19] 29
SPI_WriteData(0x00);
SPI_WriteData(0x1e);//KN3 G[18] 25
SPI_WriteData(0x00);
SPI_WriteData(0x0d);//KN4 G[17] 6
SPI_WriteData(0x00);
SPI_WriteData(0x0c);//KN5 G[16] 2
SPI_WriteData(0X00);
SPI_WriteData(0x13);//KN6 G[15] 1
SPI_WriteData(0x00);
SPI_WriteData(0x05);//KN7 G[14] 0
#endif
SPI_WriteComm(0x2a); 
SPI_WriteData(0); 
SPI_WriteData(0);
...
SPI_WriteComm(0x36);
SPI_WriteData(0x80);             //
// SPI_WriteData(0x00);             //default
// SPI_WriteData(0x20);             //设置 MV=1, x 和 y 对调 (90度旋转)
// SPI_WriteData(0xA0);             
SPI_WriteComm(0x29);
...
void lcd_blockwrite(int xs, int xe, int ys, int ye) 
{
	...
	SPI_WriteData(xs);
	SPI_WriteData(xe >> 8);
	SPI_WriteData(xe);                    //这里要注意不要超过80+25

	SPI_WriteComm(0x2b);   
	SPI_WriteData(ys >> 8);
	SPI_WriteData(ys);
	SPI_WriteData(ye >> 8);
	SPI_WriteData(ye);					//这里要注意不要超过160
...
// 画点函数，在指定的(x, y)位置绘制一个点
void DrawPoint(int x, int y, int color)
{
    // if (x >= ROW || y >= COL) return; // 确保点在LCD范围内
#if 0	
    SPI_WriteComm(0x2A); // 设置列地址
    SPI_WriteData((x+26) >> 8);  
    SPI_WriteData((x+26) & 0xFF);  
    SPI_WriteComm(0x2B); // 设置行地址
    SPI_WriteData((y+1) >> 8);
    SPI_WriteData((y+1) & 0xFF);
    SPI_WriteComm(0x2C); // 开始写入像素数据
    SPI_WriteData(color >> 8);  
    SPI_WriteData(color & 0xFF);
#else
    SPI_WriteComm(0x2A); // 设置列地址
    SPI_WriteData(x >> 8);  
    SPI_WriteData(x & 0xFF);  
    SPI_WriteComm(0x2B); // 设置行地址
    SPI_WriteData(y >> 8);
    SPI_WriteData(y & 0xFF);
    SPI_WriteComm(0x2C); // 开始写入像素数据
...
void DispColor(unsigned int color)
{
	...
	// BlockWrite(24,COL+23,0,ROW-1);
	BlockWrite(26,COL+25,1,ROW);
	...
	for(i=0;i<ROW;i++)
	{
		...
		{    
			...
			// WR0=1;  
			 ...
			 SPI_WriteData_Da(color>>8);
			 SPI_WriteData_Da(color);
		}
	}
...
void Display_row(unsigned int color)
{
	...
	// lcd_blockwrite(26,COL+25,1,ROW);
	lcd_blockwrite(26,1+25,1,1);
...
u8 the_pic_buff_test[160*8*2]={0};
void DispColor_Dma(unsigned int color)
{
	unsigned int i,j;
	u16 b_l=0;
	...
	for(i=0;i<ROW;i++)
	{
		...
		{    
			...
			// WR0=0;  
			// WR0=1;  
			 the_pic_buff_test[b_l++] = (color>>8);
			 the_pic_buff_test[b_l++] = (color);

		}
	}

	SPI_WriteData_Dma_Da(the_pic_buff_test, b_l);
...
void Display_test1_demo(void)
{
    ...
    for (int i = 0; i < (160 * 80); i++) {
        SPI_WriteData(color >> 8);  // 发送高 8 位
        SPI_WriteData(color);       // 发送低 8 位
    }
...
void DispBand(void)	 
{
	...
	// unsigned int color[8]={0x0000,0x0000,0x0000,0x0000,0x0000,0x0000,0x0000,0x0000};
	...
	//unsigned int gray16[]={0x0000,0x1082,0x2104,0x3186,0x42,0x08,0x528a,0x630c,0x738e,0x7bcf,0x9492,0xa514,0xb596,0xc618,0xd69a,0xe71c,0xffff};
   	...
   	// BlockWrite(24,COL+23,0,ROW-1);
   	// BlockWrite(24,COL+25,4,ROW+20);
   	// BlockWrite(24,COL+25,1,ROW+1);
   	BlockWrite(26,COL+25,1,ROW);
	...
	for(i=0;i<8;i++)
	{
		for(j=0;j<ROW/8;j++)
		{
	        for(k=0;k<COL;k++)
			{
				...
				WR0=0;  
				WR0=1;  */
			 SPI_WriteData_Da(color[i]>>8);
			 SPI_WriteData_Da(color[i]);
			} 
		}
	}
	...
	for(j=0;j<ROW%8;j++)
	{
        for(k=0;k<COL;k++)
		{
			...
			WR0=0;  
			WR0=1;  */
			SPI_WriteData_Da(color[7]>>8);
			SPI_WriteData_Da(color[7]);
		} 
	}
#endif
...
void DispFrame(void) 
{
	...
	SPI_WriteData_Da(0xf8);SPI_WriteData_Da(0x00);
	for(i=0;i<COL-2;i++){SPI_WriteData_Da(0xFF);SPI_WriteData_Da(0xFF);}
	SPI_WriteData_Da(0x00);SPI_WriteData_Da(0x1f);
	for(j=0;j<ROW-2;j++)
	{
		SPI_WriteData_Da(0xf8);SPI_WriteData_Da(0x00);
		for(i=0;i<COL-2;i++){SPI_WriteData_Da(0x00);SPI_WriteData_Da(0x00);}
		SPI_WriteData_Da(0x00);SPI_WriteData_Da(0x1F);
	}
...
void DispFrame_test(void) 
{
	...
	for(i=0;i<COL-2;i++){
		// SPI_WriteData_Da(0xFF);
		// SPI_WriteData_Da(0xFF);
		SPI_WriteData_Da(0x07);
		SPI_WriteData_Da(0xE0);
	}
	SPI_WriteData_Da(0x00);SPI_WriteData_Da(0x1f);
...
void EnterSLP(void)
{
	...
	SPI_WriteComm(0x0007);SPI_WriteData(0x0131); // Set D1=0, D0=1
    user_delay_us(10);
    SPI_WriteComm(0x0007);SPI_WriteData(0x0130); // Set D1=0, D0=0
    user_delay_us(10);
	SPI_WriteComm(0x0007);SPI_WriteData(0x0000); // display OFF
	//************* Power OFF sequence **************//
	SPI_WriteComm(0x0010);SPI_WriteData(0x0080); // SAP, BT[3:0], APE, AP, DSTB, SLP
	SPI_WriteComm(0x0011);SPI_WriteData(0x0000); // DC1[2:0], DC0[2:0], VC[2:0]
	SPI_WriteComm(0x0012);SPI_WriteData(0x0000); // VREG1OUT voltage
	SPI_WriteComm(0x0013);SPI_WriteData(0x0000); // VDV[4:0] for VCOM amplitude
	user_delay_us(200);                          // Dis-charge capacitor power voltage
	SPI_WriteComm(0x0010);SPI_WriteData(0x0082); // SAP, BT[3:0], APE, AP, DSTB, SLP
}
...
void ExitSLP(void)
{
	...
	//Power On sequence
	SPI_WriteComm(0x0010);SPI_WriteData(0x0080); // SAP, BT[3:0], AP, DSTB, SLP
	SPI_WriteComm(0x0011);SPI_WriteData(0x0000); // DC1[2:0], DC0[2:0], VC[2:0]
	SPI_WriteComm(0x0012);SPI_WriteData(0x0000); // VREG1OUT voltage
	SPI_WriteComm(0x0013);SPI_WriteData(0x0000); // VDV[4:0] for VCOM amplitude
	user_delay_us(200);                          // Dis-charge capacitor power voltage
	SPI_WriteComm(0x0010);SPI_WriteData(0x1490); // SAP, BT[3:0], AP, DSTB, SLP, STB
	SPI_WriteComm(0x0011);SPI_WriteData(0x0227); // R11h=0x0221 at VCI=3.3V DC1[2:0], DC0[2:0], VC[2:0]
	user_delay_us(50);                           // user_delay_us 50ms
	SPI_WriteComm(0x0012);SPI_WriteData(0x009B); // External reference voltage =Vci;
	user_delay_us(50);                           // user_delay_us 50ms
	SPI_WriteComm(0x0013);SPI_WriteData(0x1800); // R13h=0x1200 when R12=009D VDV[4:0] for VCOM amplitude
	SPI_WriteComm(0x0029);SPI_WriteData(0x0027); // R29h=0x000C when R12=009D VCM[5:0] for VCOMH
	user_delay_us(50);                           // user_delay_us 50ms
	SPI_WriteComm(0x0007);SPI_WriteData(0x0133); // 262K color and display ON
}
