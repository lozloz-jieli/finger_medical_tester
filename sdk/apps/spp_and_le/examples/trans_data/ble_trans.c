...
u8 set_ble_name[30] = {0};        //改变蓝牙名
u8 send_ble_name_to_phone[30] = {0};
u8 send_ble_name_to_phone_len;
u8 set_mac_address[6] ={0};
u8 set_mac_address_temp[6] ={0};

#define NUMBER_FRAMES_H        0          //心电数据帧数高位
#define NUMBER_FRAMES_L        1          //心电数据帧数低位
/*
...
static void trans_resume_all_ccc_enable(u16 conn_handle, u8 update_request)
{
    ...
    ble_gatt_server_characteristic_ccc_set(conn_handle, ATT_CHARACTERISTIC_FE01_01_CLIENT_CONFIGURATION_HANDLE, ATT_OP_NOTIFY);
    ble_gatt_server_characteristic_ccc_set(conn_handle, ATT_CHARACTERISTIC_FE02_01_CLIENT_CONFIGURATION_HANDLE, ATT_OP_NOTIFY);
...
void change_mac(void)
{
    ...
    {
        temp = tmp_ble_addr[i];
        tmp_ble_addr[i] = tmp_ble_addr[5-i];
        tmp_ble_addr[5-i] = temp;
    }
}
...
static uint16_t trans_att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    case ATT_CHARACTERISTIC_2A19_01_VALUE_HANDLE:
         u8 vbat_level;
        // vbat_level = app_var.notifiy_vbat_num;
         vbat_level = get_vbat_percent();
        //vbat_level = my_qhf_get_vbat_percent();
        g_printf("vbat_level = %d",vbat_level);
        ...
        if (buffer) {
            memcpy(buffer, &vbat_level, buffer_size);
            att_value_len = buffer_size;
        }        
    ...
    case ATT_CHARACTERISTIC_2A23_01_VALUE_HANDLE:          
        ...
        if (buffer) {
            memcpy(buffer, tmp_ble_addr, buffer_size);
            att_value_len = buffer_size;
        }

        break;
   case ATT_CHARACTERISTIC_2D37_01_VALUE_HANDLE:           //读取心电数据
    ...
    case ATT_CHARACTERISTIC_2DF7_01_VALUE_HANDLE: 
        
            att_value_len = send_ble_name_to_phone_len;
            y_printf("-----read_call_back_ble_name-------\n");
       
      
          
            put_buf(send_ble_name_to_phone,send_ble_name_to_phone_len);
            
            y_printf("-----read_call_back_ble_name-------okokkokokok\n");
           
           if(app_var.flag_change_ble_name == 1)
           {
                // read_callback_blename_poweroff_timer(); //开个定时器，readcallback两秒后关机
           }   
            ...
            if (buffer) {
                memcpy(buffer, send_ble_name_to_phone, buffer_size);
                att_value_len = buffer_size;
            }
...
static int trans_att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    case ATT_CHARACTERISTIC_ae3c_01_CLIENT_CONFIGURATION_HANDLE:
    case ATT_CHARACTERISTIC_2a05_01_CLIENT_CONFIGURATION_HANDLE:
    case ATT_CHARACTERISTIC_FE01_01_CLIENT_CONFIGURATION_HANDLE:
    case ATT_CHARACTERISTIC_FE02_01_CLIENT_CONFIGURATION_HANDLE:
    ...
    case ATT_CHARACTERISTIC_2DF7_01_VALUE_HANDLE:           //修改设备蓝牙名称及MAC地址
        r_printf("----change-----name---adderss\n");
        put_buf(buffer, buffer_size);
        memset(set_ble_name,0,sizeof(set_ble_name));
        memset(send_ble_name_to_phone,0,sizeof(send_ble_name_to_phone));
        memset(set_mac_address,0,sizeof(set_mac_address));
        memcpy(send_ble_name_to_phone,buffer,buffer_size);       //send_ble_name_to_phone 将此上传至手机 readcallback
        send_ble_name_to_phone_len = buffer_size;
        app_var.set_ble_name_len=buffer_size;                    //mac地址前面的都是蓝牙名
        memcpy(set_ble_name,buffer,app_var.set_ble_name_len-6);     //取蓝牙名前部分

        memcpy(set_mac_address,&buffer[buffer_size-6],6);     //取mac地址部分

        u8 str_temp[13];
       
        sprintf(str_temp, "%02X%02X%02X%02X%02X%02X",buffer[buffer_size-6],buffer[buffer_size-5],buffer[buffer_size-4],buffer[buffer_size-3],buffer[buffer_size-2],buffer[buffer_size-1]);
    
        memcpy(&set_ble_name[app_var.set_ble_name_len-6],str_temp,13);     //取mac地址部分
        app_var.set_ble_name_len += 6;
        r_printf("set_ble_name_len:%d\n",app_var.set_ble_name_len);
        //将长度和蓝牙名写进去
        write_name_len_vm();
        write_vm_name();
        write_mac_address_vm();
        
        app_var.reset_factory = 1;
        write_vm_reset_factory(); 

        // ble_module_enable(0);   //开广播
        // bt_ble_init();          //关广播
        ...
        //ble_comm_att_send_data(connection_handle, ATT_CHARACTERISTIC_2DF7_01_VALUE_HANDLE, set_ble_name, app_var.set_ble_name_len, ATT_OP_NOTIFY);   //将修改后的蓝牙名上发至上位机
       // y_printf("notify ok!\n");
        put_buf(set_ble_name,app_var.set_ble_name_len);
        app_var.flag_change_ble_name = 1;
       // power_set_soft_poweroff();              //关机
        break;

    case ATT_CHARACTERISTIC_FE01_01_VALUE_HANDLE: 
        app_write_cmd_control(buffer,buffer_size);
        break;
...
void Get_ECG_ad_value_deal(u16 adv_value)
{
    u32 ret;
    u32 ret_1;
    static u32 ret_last_time;

    if(app_var.bt_conn != 1){
        app_var.real_flag = 0;
        return;
    }

    if(app_var.real_flag != 1){                     // 没有app开始采集指令
        return;
    }

    static u8 ECG_BUF[20];     //上发心电数据包
    static u8 cnt_adc_value=0;   //位数
 

    ECG_BUF[NUMBER_FRAMES_H] = (app_var.Number_frames>>8) & 0xFF;         //帧数高位存入ECG_BUF[0]
    ECG_BUF[NUMBER_FRAMES_L] = app_var.Number_frames & 0xFF;              //帧数低位存入ECG_BUF[1]

    cnt_adc_value += 2;
    //ret = adc_get_voltage(TCFG_ADECG_AD_CHANNEL);                         //将获取的数据存入数据包
    // ret = adc_sample_ch_one(IO_PORTA_07,AD_CH_PA7);
    //ret = adc_fast_sample(AD_CH_PA7);
    ...
    ECG_BUF[cnt_adc_value] = (ret>>8) & 0xFF;  
    ECG_BUF[cnt_adc_value+1] = ret & 0xFF;  
  
    if(cnt_adc_value == 18)
    {       
        app_var.Number_frames++;
        cnt_adc_value = 0;
        
        ble_comm_att_send_data(trans_con_handle, ATT_CHARACTERISTIC_2D37_01_VALUE_HANDLE, ECG_BUF,sizeof(ECG_BUF), ATT_OP_NOTIFY);    //notify
...
//mcu发送电量给app
void mcu_send_electricity_data_to_app(void)
{
    y_printf("-------power------data---------\n");
    u8 electricity_data = 0;    
    u8 hex_electricity_data[1];  
    electricity_data = my_qhf_get_vbat_percent();
    sprintf(hex_electricity_data, "%x", electricity_data);              
    put_buf(hex_electricity_data, sizeof(hex_electricity_data));
    // ble_comm_att_send_data(trans_con_handle, ATT_CHARACTERISTIC_FFE2_01_VALUE_HANDLE, electricity_data, sizeof(electricity_data), ATT_OP_WRITE_WITHOUT_RESPOND);
    ble_comm_att_send_data(trans_con_handle, ATT_CHARACTERISTIC_2A19_01_VALUE_HANDLE, hex_electricity_data, sizeof(hex_electricity_data), ATT_OP_NOTIFY);
}


void mcu_send_control_app(u8 *buffer,u16 len)
{
    ble_comm_att_send_data(trans_con_handle, ATT_CHARACTERISTIC_FE01_01_VALUE_HANDLE, buffer, len, ATT_OP_NOTIFY);
}

void mcu_send_history_app(u8 *buffer,u16 len)
{
    ble_comm_att_send_data(trans_con_handle, ATT_CHARACTERISTIC_FE02_01_VALUE_HANDLE, buffer, len, ATT_OP_NOTIFY);
}
/*************************************************************************************************/
/*!
...
int write_vm_reset_factory(void)
{
    int ret;
    ret = syscfg_write(CFG_USER_FARESET,&app_var.reset_factory,1);
    if(ret < 0){
        r_printf("write vm baud error");
    }
    return ret;
}

int read_vm_reset_factory(void)
{
    int ret;
    ret = syscfg_read(CFG_USER_FARESET,&app_var.reset_factory,1);
    if(ret < 0){
        r_printf("read vm app_var.reset_factory error");
    }
    r_printf("app_var.reset_factory = %d",app_var.reset_factory);
    return ret;

}


int write_vm_name(void)
{
    int ret;
    ret = syscfg_write(CFG_USER_NAME,set_ble_name,app_var.set_ble_name_len);
    if(ret < 0){
        r_printf("write vm name error");
    }
    r_printf("write app_var.at_at_name_len :%d",app_var.set_ble_name_len);
    r_printf("write name vm %s",set_ble_name);

    return ret;
}


int write_name_len_vm(void)
{   
    int ret;
    ret = syscfg_write(CFG_USER_NAME_LEN,&app_var.set_ble_name_len,4);
    if(ret < 0){
        r_printf("write vm name len error");
    }
   
    y_printf("write name len = %d",app_var.set_ble_name_len);
     return ret;    
}



int read_vm_name(void)
{
    int ret;
    ret = syscfg_read(CFG_USER_NAME,set_ble_name,app_var.set_ble_name_len);
    if(ret < 0){
        r_printf("read vm name error");
    }
    r_printf("----%s",set_ble_name);
    return ret;
...
int read_name_len_vm(void)
{
    int ret;
    ret = syscfg_read(CFG_USER_NAME_LEN,&app_var.set_ble_name_len,4);
    if(ret < 0){
        r_printf("read vm name len error");
    }
    y_printf("read name len = %d",app_var.set_ble_name_len);
    return ret;    
}
...
int write_mac_address_vm(void)
{   
    int ret;
    ret = syscfg_write(CFG_USER_MAC_ADDRESS,set_mac_address,6);
    if(ret < 0){
        r_printf("write mac address len error");
    }
...
int read_mac_address_name(void)
{
    int ret;
    ret = syscfg_read(CFG_USER_MAC_ADDRESS,set_mac_address,6);
    if(ret < 0){
        r_printf("read mac address error");
    }
    r_printf("----%s",set_ble_name);
    return ret;
...
void chang_mac_test(void)
{
    // set_mac_address[0] = 0x11;
    // set_mac_address[1] = 0x11;
    // set_mac_address[2] = 0x11;
    // set_mac_address[3] = 0x11;
    // set_mac_address[4] = 0x11;
    // set_mac_address[5] = 0x11;
    
    // write_mac_address_vm();

    // le_controller_set_mac((void *)set_mac_address);
    btstack_exit();
